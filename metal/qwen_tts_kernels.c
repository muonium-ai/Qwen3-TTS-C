...
void kernel_transposed_conv1d(float *out, const float *input, const float *weight,
     ...
     * Weight layout [in_channels, out_channels, kernel_size] → repack per tap.
     */
    {
...
#pragma omp parallel for schedule(static)
            ...
            for (int k = 0; k < kernel_size; k++) {
                /* Repack weight slice: wk_packed[oc, ic] = weight[ic, oc, k] */
                for (int oc = 0; oc < out_channels; oc++) {
                    for (int ic = 0; ic < in_channels; ic++) {
                ...
                /* GEMM: temp[out_channels, n] = wk_packed[out_channels, in_channels] @ input[in_channels, length]
                 * Only first n columns of input matter (input stride = length for column layout). */
                cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
                            out_channels, n, in_channels,
                ...
                /* Scatter: out[oc, k + t*stride] += temp[oc, t] */
#ifdef USE_OPENMP
#pragma omp parallel for schedule(static)
#endif
                for (int oc = 0; oc < out_channels; oc++) {
